...
defineOptions({ name: 'RpwResourcePlanEquipmentForm' });

const emit = defineEmits(['success']);
const formData = ref<ResourcePlanEquipment>();
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    modalApi.lock();
    // 提交表单
    const data = (await formApi.getValues()) as ResourcePlanEquipment;
    try {
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    const data = modalApi.getData<ResourcePlanEquipment>();
    if (!data || !data.id) {
      return;
    }
    // 加载数据
    modalApi.lock();
    try {
      formData.value = await equipmentApi.get(data.id);
      // 设置到 values
