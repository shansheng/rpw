...
/** Zoom：恢复（适配视口）
 *  注意：容器或图元尚未完成布局时，bpmn-js 的 fit-viewport 会算出非有限缩放
 *  (Infinity / NaN) 并抛 "non-finite"。这里改用 requestAnimationFrame 等浏览器
 *  完成一次布局后再适配，并对异常做兜底（重试 / 降级 1:1），避免整块流程图渲染崩溃。
 */
const processReZoom = () => {
  defaultZoom.value = 1;
  ...
  const fit = () => {
    try {
      ...
      // 布局尚未就绪：稍后重试；多次失败则降级为 1:1 保证图可见
      if (tries++ < 3) {
...
const setProcessStatus = (view: any) => {
  ...
  // 本 fork 后端直接返回扁平的 ProcessInstanceVO，其本身即流程实例上下文
  // （与 yudao 原始「嵌套 processInstance」结构不同，故不再判断 view.processInstance）
  ...
  // 起始节点恒标记为「已完成」，让流程图起点更直观
  elementRegistry
    .filter((element: any) => element.type === 'bpmn:StartEvent')
    .forEach((item: any) => {
      if (
        !canvas.hasMarker(item.id, 'danger') &&
        !canvas.hasMarker(item.id, 'cancel')
      ) {
        canvas.addMarker(item.id, 'success');
      }
    });

  // 特殊：处理 end 节点的高亮。取消 / 驳回时，end 节点用对应颜色
  if (
