...
/**
 * 兜底抓取：当「流程设计」步骤的设计器已挂载时，主动把最新 BPMN XML（或 SIMPLE 的 simpleModel）
 * 写回 formData，避免依赖单一响应式回写链路遗漏，导致发布时报“模型 BPMN 为空”。
 * 注意：若用户从未进入「流程设计」步骤（设计器未挂载），则无法抓取，仍会触发下方空校验。
 */
async function syncProcessData() {
  if (formData.value.type === BpmModelType.BPMN) {
    const xml = await processDesignRef.value?.getProcessData?.();
    if (xml) {
      formData.value.bpmnXml = xml;
    }
  }
}
...
async function handleSave() {
  try {
    ...
    // 兜底：设计器已挂载时主动抓取最新流程数据，避免响应式回写遗漏导致发布报“BPMN 为空”
    await syncProcessData();
    ...
    switch (actionType) {
      ...
      default: {
        // 情况四：新增场景
        // 幂等：若已创建过（id 已存在），改为更新，避免重复点击“保存”生成多条记录
        if (formData.value.id) {
          await updateModel(modelData);
          message.success('保存成功，可点击【发布】按钮，进行发布模型');
        } else {
          formData.value.id = await createModel(modelData);
          // 提示成功
...
async function handleDeploy() {
  try {
    ...
    // 1.2.1 兜底：设计器已挂载时主动抓取最新流程数据，避免响应式回写遗漏
    await syncProcessData();

    // 1.3 BPMN 类型必须有流程图（bpmnXml）。若用户从未进入「流程设计」或未绘制完整流程，给出明确引导
    if (
      formData.value.type === BpmModelType.BPMN &&
      !formData.value.bpmnXml
    ) {
      message.warning('请先在「流程设计」中绘制完整的流程图（需包含开始与结束节点）后再发布');
      return;
    }
  ...
  <Page auto-content-height>
    <div class="mx-auto">
      ...
      <Card :body-style="{ padding: '10px' }" class="mb-4">
        <div class="mt-12">
          <!-- 第一步：基本信息（使用 v-show 保持挂载，保证任意步骤点击“保存”时 basicInfoRef 仍存在，分类必填校验不被绕过） -->
          <div v-show="currentStep === 0" class="mx-auto w-4/6">
            <BasicInfo
              v-model="formData"
