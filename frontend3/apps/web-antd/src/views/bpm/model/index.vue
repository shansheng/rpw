...
async function getList() {
  ...
  try {
    ...
    // 未分类（无分类或分类已被删除）的模型也要展示，
    // 否则“保存成功”却因列表按分类分组过滤而在列表中看不到
    const uncategorized = modelList.filter((model: any) => !model.categoryName);
    if (uncategorized.length > 0) {
      categoryGroup.value.push({
        id: -1,
        name: '未分类',
        modelList: uncategorized,
      } as any);
    }
    // 重置排序实例
...
/** 初始化：onMounted 覆盖普通进入场景，onActivated 覆盖 keepAlive 从创建页返回场景，双保险确保列表一定刷新 */
onMounted(() => {
  getList();
});
onActivated(() => {
