...
/** 是否为“未分类”虚拟分组（id = -1），该组不允许重命名/删除分类 */
const isUncategorized = computed(() => props.categoryInfo.id === -1);
...
/** 监听分类信息变化，立即同步到 Grid（immediate 确保挂载即用最新数据，避免 v-if 延迟挂载导致数据丢失） */
watch(
  () => props.categoryInfo,
  () => {
    updateModelList();
              ...
              <Dropdown placement="bottom" arrow>
                ...
                <template #overlay>
                  <Menu @click="(e) => handleCommand(e.key as string)">
                    <Menu.Item v-if="!isUncategorized" key="renameCategory"> 重命名 </Menu.Item>
                    <Menu.Item v-if="!isUncategorized" key="deleteCategory"> 删除分类 </Menu.Item>
                  </Menu>
                </template>
              </Dropdown>
            </template>
