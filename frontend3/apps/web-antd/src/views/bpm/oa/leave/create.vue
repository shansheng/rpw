...
const processDefineKey = 'p_Leave'; // 流程定义 Key（对应后端已部署的请假流程 p_Leave）
...
async function onSubmit() {
  ...
  const submitData: LeaveCreateData = {
    ...data,
    // 请假类型为单选，ant-design-vue Select 在部分场景下会把值以数组形式（如 ["1"]）返回，
    // 归一化为标量字符串，避免后端 Jackson 反序列化 String 字段时报 "from Array value"
  ...
  try {
    ...
    await router.push({
      // 列表页后端菜单(id=86)注册路径为 /bpm/oa/leave/index（path=oa/leave/index + 父级 /bpm），
      // 不能漏掉 /index 后缀，否则路由不匹配落到 404。
      path: '/bpm/oa/leave/index',
    });
  } finally {
