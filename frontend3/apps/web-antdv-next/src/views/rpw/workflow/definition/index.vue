...
async function handleViewXml(row: WorkflowApi.ProcessDefinitionDTO) {
  ...
  <Page auto-content-height>
          ...
          :actions="[
            {
              label: '查看BPMN',
              type: 'link',
              onClick: handleViewXml.bind(null, row),
            },
            {
              label: '删除',
              type: 'link',
    ...
    >
      <div class="space-y-2 p-1">
        <span v-if="xmlLoading" class="text-gray-400">加载中…</span>
        <pre
          v-else
          class="max-h-[60vh] overflow-auto rounded bg-gray-50 p-3 text-xs leading-relaxed"
        >{{ xmlContent || '（无内容）' }}</pre>
      </div>
    </Modal>
  </Page>
