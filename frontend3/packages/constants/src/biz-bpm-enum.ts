 ...
 * 流程实例状态
 *
 * ⚠️ 此对象只是「逻辑判断常量」，数值必须与后端字典 `bpm_process_instance_status` 严格一致：
 *    1=审批中 / 2=审批通过 / 3=已取消 / 4=审批不通过（以 BpmProcessInstanceStatusEnum 存储为准）。
 * 仅用于 `status === BpmProcessInstanceStatus.XXX` 之类的按钮显隐/分支判断；
 * 中文 label（审批中/审批通过/已取消/审批不通过）一律经 CellDict / getDictOptions 从后端字典获取，
 * 切勿在本处硬编码中文 label，否则会与后端字典脱节、显示错乱。
 */
