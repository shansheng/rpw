<template>
  <div class="resource-plan-safety-container">
    <el-card>
      <template #header>
        <div class="card-header">
          <span>安全物资计划管理</span>
          <el-button type="primary" @click="handleAdd">新增安全物资计划</el-button>
        </div>
      </template>

      <!-- 搜索表单 -->
      <el-form :inline="true" class="search-form">
        <el-form-item label="安全物资名称">
          <el-input
              v-model="searchForm.safetyName"
              placeholder="请输入安全物资名称"
              clearable
      ...
      <el-table :data="tableData" v-loading="loading" border style="width: 100%">
        ...
        <el-table-column prop="safetyName" label="安全物资名称" min-width="150" />
    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="安全物资名称" prop="safetyName">
          <el-input v-model="form.safetyName" placeholder="请输入安全物资名称" />
        </el-form-item>
...
import {
  getResourcePlanSafetyList,
  getResourcePlanSafetyById,
  createResourcePlanSafety,
  updateResourcePlanSafety,
  deleteResourcePlanSafety
} from '@/api/resourcePlanSafety'

interface ResourcePlanSafety {
  id?: number
  projectId?: number
  wbsCode: string
  safetyName: string
...
// 数据列表
const tableData = ref<ResourcePlanSafety[]>([])
...
const dialogTitle = ref('新增安全物资计划')
const isEdit = ref(false)
const currentId = ref<number>()

// 表单
const formRef = ref<FormInstance>()
const form = reactive<ResourcePlanSafety>({
  projectId: undefined,
  wbsCode: '',
  safetyName: '',
  specification: '',
...
const rules: FormRules = {
  ...
  safetyName: [{ required: true, message: '请输入安全物资名称', trigger: 'blur' }]
}
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await getResourcePlanSafetyList()
    if (res.data) {
...
const handleAdd = () => {
  resetForm()
  dialogTitle.value = '新增安全物资计划'
  isEdit.value = false
  dialogVisible.value = true
}

// 编辑
const handleEdit = async (row: ResourcePlanSafety) => {
  resetForm()
  dialogTitle.value = '编辑安全物资计划'
  isEdit.value = true
  currentId.value = row.id

  try {
    const res = await getResourcePlanSafetyById(row.id!)
    if (res.data) {
...
const handleDelete = (id: number) => {
  ElMessageBox.confirm('确定删除该安全物资计划吗？', '提示', {
    type: 'warning'
  }).then(async () => {
    try {
      const res = await deleteResourcePlanSafety(id)
      if (res.data) {
    ...
    if (valid) {
      try {
        ...
        if (isEdit.value && currentId.value) {
          const res = await updateResourcePlanSafety(currentId.value, { ...form })
          success = res.data || false
        } else {
          const res = await createResourcePlanSafety({ ...form })
          success = res.data || false
        }
...
<style scoped>
.resource-plan-safety-container {
  padding: 20px;
}
