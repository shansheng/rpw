<template>
  <div class="resource-plan-hardware-container">
    <el-card>
      <template #header>
        <div class="card-header">
          <span>五金计划管理</span>
          <el-button type="primary" @click="handleAdd">新增五金计划</el-button>
        </div>
      </template>
      ...
      <el-table :data="tableData" v-loading="loading" border style="width: 100%">
        ...
        <el-table-column prop="hardwareName" label="五金名称" min-width="150" />
    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="五金名称" prop="hardwareName">
          <el-input v-model="form.hardwareName" placeholder="请输入五金名称" />
        </el-form-item>
...
import {
  getResourcePlanHardwareList,
  getResourcePlanHardwareById,
  createResourcePlanHardware,
  updateResourcePlanHardware,
  deleteResourcePlanHardware
} from '@/api/resourcePlanHardware'

interface ResourcePlanHardware {
  id?: number
  projectId?: number
  wbsCode: string
  hardwareName: string
...
// 数据列表
const tableData = ref<ResourcePlanHardware[]>([])
...
const dialogTitle = ref('新增五金计划')
const isEdit = ref(false)
const currentId = ref<number>()

// 表单
const formRef = ref<FormInstance>()
const form = reactive<ResourcePlanHardware>({
  projectId: undefined,
  wbsCode: '',
  hardwareName: '',
  specification: '',
...
const rules: FormRules = {
  ...
  hardwareName: [{ required: true, message: '请输入五金名称', trigger: 'blur' }]
}
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await getResourcePlanHardwareList()
    if (res.data) {
...
const handleAdd = () => {
  resetForm()
  dialogTitle.value = '新增五金计划'
  isEdit.value = false
  dialogVisible.value = true
}

// 编辑
const handleEdit = async (row: ResourcePlanHardware) => {
  resetForm()
  dialogTitle.value = '编辑五金计划'
  isEdit.value = true
  currentId.value = row.id

  try {
    const res = await getResourcePlanHardwareById(row.id!)
    if (res.data) {
...
const handleDelete = (id: number) => {
  ElMessageBox.confirm('确定删除该五金计划吗？', '提示', {
    type: 'warning'
  }).then(async () => {
    try {
      const res = await deleteResourcePlanHardware(id)
      if (res.data) {
    ...
    if (valid) {
      try {
        ...
        if (isEdit.value && currentId.value) {
          const res = await updateResourcePlanHardware(currentId.value, { ...form })
          success = res.data || false
        } else {
          const res = await createResourcePlanHardware({ ...form })
          success = res.data || false
        }
...
<style scoped>
.resource-plan-hardware-container {
  padding: 20px;
}
